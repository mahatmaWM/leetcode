...
import sys
sys.path.append('../../')
from data_structure.stack.arrayStack import ArrayStack  # 我们实现的栈
from data_structure.queue.loopqueue import LoopQueue  # 我们实现的循环队列
    ...
    def _remove(self, node, elem):
        ...
        if elem < node.elem:  # 要寻找的元素小于当前节点的elem值
            ...
            if node.left is None:  # node左子树为空的情况，单独处理，与前面的删除最大/最小节点的方法一致，不再赘述
                ...
                successor.right = self._removeMin(
                    node.right)  # 通过self._removeMin方法将node的右子树的最小节点删除，注意返回的删除节点的新的右子树的根节点，所以
    ...
    def _ceil(self, node, elem):
        ...
        if elem < node.elem:
            ...
            if node.right is None:  # 此时node没有右子树，也就没有什么所谓的ceil
                ...
                return self._minimum(node.right)  # 调用self._minimum函数找到node右子树中携带最小值的节点
